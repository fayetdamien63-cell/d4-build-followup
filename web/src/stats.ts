import { ancestralKey, boardKeys, countDone, gearKeys, masterworkKey } from '../../shared/progress.ts'
...
const TIERS = ['Obtenir', 'Aspect', 'Primordial', 'Affixe', 'Trempe', 'Châsse', 'Masterwork'] as const
...
export function gearTodos(v: Variant, isDone: (k: string) => boolean): GearTodo[] {
  ...
  for (const slot of v.gear) {
    ...
    for (const a of slot.affixes) if (!isDone(a.key)) todos.push({ key: a.key, slot, tier: 3, label: a.text, detail: a.greater ? 'Greater' : undefined })
    for (const a of slot.tempered) if (!isDone(a.key)) todos.push({ key: a.key, slot, tier: 4, label: a.text })
    for (const s of slot.sockets) if (!isDone(s.key)) todos.push({ key: s.key, slot, tier: 5, label: s.name })
    if (!isDone(masterworkKey(slot))) todos.push({ key: masterworkKey(slot), slot, tier: 6, label: 'Masterwork terminé' })
  }
